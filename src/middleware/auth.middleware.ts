...
function hasEditorRole(decodedToken: auth.DecodedIdToken): boolean {
  return decodedToken.role === "editor" || decodedToken.role === "admin";
}

/**
 * Middleware to verify Firebase Auth token and check editor authorization
 *
 * Usage:
 *   app.post('/editor-only-route', verifyAuthenticatedEditor(logger), handler)
 *
 * Sets req.user with { uid, email, email_verified } if authenticated and authorized
 */
export function verifyAuthenticatedEditor(logger?: SimpleLogger) {
  ...
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      log.info("Editor authenticated successfully", {
        requestId,
        email,
        uid,
      });
